...
function makeEl(el_name, html_str, target, method) {
  ...
  if ( target && isElementNode(target) && target.parentNode ) {
    switch( method ){
      ...
      break;
      case 'prepend' : target.insertAdjacentElement('afterbegin', el);
      break;
      case 'before' : target.insertAdjacentElement('beforebegin', el);
      break;
      case 'after' : target.insertAdjacentElement('afterend', el);
      break;
    }
    // if ( method === 'append' ) {
...
function first(node){
  // if(!isElementNode(node)){console.error(''); return;}
